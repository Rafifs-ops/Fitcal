...
const statusLogin = localStorage.getItem("isLogin"); // Mendapatkan data statusLogin dari localstorage
...
const data = useData() // Mengambil state dari pinia
const { users } = storeToRefs(data); // Mendapatkan data users dari pinia dan menjaga reaktivitas
onMounted(async () => {
  await data.fetchUsers(); // Menjalankan fungsi yang ada di dalam state pinia untuk mendapatkan data
})
...
function login() {
  const getUsername = username.value; // Mendapatkan data username dari form login
  const getPassword = password.value; // Mendapatkan data password dari form login

  // VALIDASI LOGIN

  const getData = users.value.find(user => user.username === getUsername && user.password === getPassword);
  // output : single objek
  // user adalah nilai satu per satu dari users.value

  if (getData) {
    localStorage.setItem("username", getData.username); // Menambahkan data localStorage berupa username
    localStorage.setItem("isLogin", true); // Menambahkan data localStorage berupa password
